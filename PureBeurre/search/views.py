...
def substitutes_products(request, product_id):
    ...
    set_of_substitutes = product.category.product_set.filter(nutrition_score__lt=product.nutrition_score).order_by('nutrition_score')[:9]
    return render(request, 'search/substitutes.html', {'product' : product, 'substitutes' : set_of_substitutes})
