...
@login_required(login_url='login') # user has to be logged if he want to see favorites
def user_favorites(request):
    """
    This view allows user to see favorites linked with his account
    """
    current_user_id = request.session.get("_auth_user_id") #We retrieve user id
...
def add_favorite(request, product_id, substitute_id):
    ...
    user = User.objects.get(id=current_user_id)
    try:
        product, substitute = Product.objects.get(barcode=product_id), Product.objects.get(barcode=substitute_id)
